...
1.Syntax errors
2.mispelled vars
3.some faulty logic with else if in wrong place and else in wrong place
*/
   ...
   if(birthMo > HIGHMO){
      printf( "Invalid month\n");
         ...
         if(birthDay <= HIDAY3){
           ...
           printf( "Invalid day for month in %d %d\n",  birthMo, birthYr );
        }
    }else if(birthMo == 2){
        if(birthDay <= HIDAY4){
           printf( "Valid day\n");
    ...
    }else if (birthMo < 8 && birthMo%2 == 1 || birthMo > 7 && birthMo%2 == 0){
        if(birthDay <= HIDAY1){
            printf( "Valid day"  );
        }else{
	...
	printf("here\n");
        if(birthMo > 3  && birthMo%2 == 0 && birthMo < 7 && birthDay <= HIDAY2 || birthMo > 8 && birthMo%2 == 1 && birthDay <= HIDAY2){
            printf( "Valid day\n");
        }else{
            printf( "Invalid day for month %d in year  %d\n" , birthMo, birthYr );
        }
   }
   return '0';
}
