...
char* gamewindow_information(gamewindow_t *window){
	char* output = malloc(30 * sizeof(char)); // init an array to fit 10 strings
  	gamewindow_t *temp = window; // temp object mostly for ease of copying code that works similarly

	// get snake information and add to the output
	char * tempstr = malloc(13 * sizeof(char));
	sprintf(tempstr, "%d,%d,%d,%d,%c,%d,%d,%d\n", temp->upper_left_x, temp->upper_left_y, temp->width, temp->height, temp->draw_char, temp->color[0], temp->color[1], temp->color[2]);
